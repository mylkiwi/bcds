...
- 均衡覆盖：红球综合历史频次、近 20 期频次、遗漏和随机扰动；蓝球按 1/16 均匀概率做弱加权。
- 热号偏向：提高红球历史高频和近期高频权重；蓝球只在频率偏差有统计证据时轻微倾斜。
- 遗漏回补：提高红球长时间未出号码权重；蓝球长期遗漏只展示，不作为回补依据。
- 冷号逆向：提高红球低频和长遗漏号码权重；蓝球冷门只用于覆盖分散。
- 冷热混合：红球热号、遗漏和随机扰动混合；蓝球接近均匀底池。
- 纯随机：不使用走势权重，只保留基础形态过滤。

组合形态过滤会检查奇偶、大小区、三区分布、和值、连号。避开大众号码会降低生日号集中、长连号、同尾号过多、上期重号过多、全奇/全偶、全大/全小组合的权重。

低重叠组合池会按同一策略生成多组备选，并限制组间红球重复数量，适合多组投注时减少重复覆盖。蓝球策略默认每个号码都是 1/16 基础概率，只用二项分布偏差给很弱的统计信号；星标只来自频率偏差证据，单纯长期遗漏不会触发星标。

逐期反推规则来自滚动历史对比：红球优先靠近 1-2 个前 30 期高频号、1-2 个久未出号、1-2 个上期附近号；蓝球主要靠多蓝球覆盖和组间分散，不把遗漏当成“更该出”。
