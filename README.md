...
本地直接运行一体化服务（已有 `.env` 会自动读取，不需要在网页填写密钥）：

```bash
./run_local.sh
```

再打开 `http://127.0.0.1:8000/`。页面默认使用近 100 期、本地综合研究、复式 7 红 + 2 蓝、形态过滤和避开大众号码；参数折叠为可选项，打开即可使用“本地生成”或“AI 分析生成”。旧 AI 报告不会在刷新时覆盖默认设置，仍可手动查看。

**两项密钥分工不同，不要填成同一个值：**

| 配置 | 用途 | 本地默认行为 |
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | 服务端请求 DeepSeek | 读取已有 `.env`；不发送到浏览器、不在页面输入 |
| `SSQ_ADMIN_TOKEN` | 保护购买记录、核奖和 AI 报告删除；AI 生成与读取不需要它 | 缺失、示例占位值或与 AI 密钥重复时，自动生成独立随机值并保存到权限为 `600` 的 `.env` |

首次接入 DeepSeek 仍需要有效的服务商密钥，程序不能替你生成；缺少它时本地选号和购买记录仍可用，AI 明确显示未配置。

`run_local.sh` 从任意目录启动都会定位到项目目录，启用仅本机的自动连接。该模式同时检查实际监听地址、客户端地址、Host、Origin、浏览器来源及非简单请求头，拒绝代理转发和跨站请求；密钥不会写入 HTML / JavaScript 或本地浏览器存储。公网 / Docker 默认**不启用**本地自动授权；AI 生成与读取无需网站授权，购买记录、核奖和 AI 报告删除仍需独立管理密钥解锁。已有购买密钥可继续复用，在 HTTPS 页面首次解锁后本浏览器记住 30 天；不要把购买及删除接口公开成免鉴权。

购买记录的期号按当前已加载数据推导，号码、备注默认跟随当前推荐，表单折叠为可选编辑；手动编辑后不会被新推荐悄悄覆盖，可点击“填入当前推荐”恢复跟随。只有点击“保存购买记录”才会写入记录，本工具不会代购或自动下单。记录前应确认加载的开奖数据已更新。

页面、历史数据和 `/api/ai/*` 使用同一来源，AI 请求由 Python 服务端携带密钥调用真实 DeepSeek API。直接打开 `index.html` 仍可查看静态分析，但接口操作会提示使用上面的本地服务地址。
...
AI 生成、任务查询及报告读取无需 `SSQ_ADMIN_TOKEN`，只有 AI 报告删除仍需购买授权。公开 AI 调用保留单请求互斥、最短调用间隔、每日额度、64KB 请求上限和上游超时；知道网址的人也能消耗 AI 额度，免授权并不限制为本人使用。默认使用 `deepseek-v4-flash`，可通过环境变量调整：
...
默认调用参数也由服务端准备：分析阶段采用非思考 JSON 输出（4000 token、至少 90 秒超时）；选号阶段保留思考模式，但使用低推理强度和 16000 token 输出额度（至少 180 秒超时），为完整结构化结果留出空间。网络不通、响应超时、HTTP 状态和无效 JSON 分开提示；遇到输出截断仍拒绝使用残缺号码，不会将部分内容伪装成成功报告。
...
公开的静态数据文件只有历史开奖（AI 研究报告另外通过免授权 API 读取，购买记录不公开）：
...
生产环境所有购买记录接口都需要管理密钥。本地自动连接仅在 `run_local.sh` 启动的本机服务中启用。部署环境变量：
...
- **AI 生成、任务查询、报告列表和详情无需网站授权**，默认参数直接使用。AI 提供商密钥仍只放在服务端；网页不接收、不保存该密钥。AI 报告为公开研究结果，请勿在报告中放入私人信息。
- **购买记录的查看、保存、删除、核奖必须授权**。线上 HTTPS 页面首次点击“解锁购买记录”，填写独立的 `SSQ_ADMIN_TOKEN`（不是 AI 密钥）。服务端签发有效期 30 天的随机会话，浏览器使用 `HttpOnly; Secure; SameSite=Strict`、Host-only Cookie；不再持久保存或随每次请求发送原始管理密钥。不同浏览器分别解锁。
- “锁定购买记录 / 退出授权”立即撤销当前会话；会话到期或服务端更换购买密钥后需重新解锁。会话仅以摘要保存到私有 SQLite，正常容器重建后仍有效。旧版浏览器管理密钥仅尝试迁移一次并从 `localStorage` 清除。
- AI 历史的**删除**属于管理操作，仍需购买授权，避免匿名删除报告。购买锁定或服务异常不阻断 AI 读取、生成。
- `GET /api/session` 公开返回配置布尔值和本浏览器授权状态；`POST /api/session` 解锁；`DELETE /api/session` 退出。兼容接口 `GET /api/access` 及显式 Bearer/X-Admin-Token 管理调用继续鉴权。所有状态接口永不返回密钥值。
- 本地 `run_local.sh` 继续只在严格验证的同源回环连接上自动授权，不保存、不发送管理密钥；生产环境不会因 nginx 的回环连接获得自动授权。生产会话必须使用 HTTPS，不能根据代理的 HTTP 内部连接降级 Cookie。
- 不开放跨站 CORS；写操作校验 Origin、Sec-Fetch-Site 和 JSON 请求类型。解锁失败全局 5 分钟最多 10 次。AI 仍保留单任务并发、最小调用间隔和每日额度；这些额度及解锁限流当前在进程内计数，重启会重置。**免授权不等于仅自己可访问**：知道网址的人也可以调用 AI、消耗服务端额度；不要把网址当成访问控制。

### 前端版本与发布

入口 JS/CSS URL 使用文件内容摘要，数据加载成功和失败两条路径保持同版。Nginx 对静态入口返回 `Cache-Control: no-cache, must-revalidate`，API 保持 `no-store`，避免不同浏览器混用新页面和旧脚本。

修改 JS/CSS 后先运行 `python3 update_asset_versions.py`，再运行测试、提交和推送；镜像构建也会自动校准版本。发布只替换双色球镜像，保留私有数据卷和密钥配置，发布前备份私有数据，不上传本地个人购买记录。
...
node --check app.js
node --test test_frontend_defaults.mjs
```

覆盖本地免输入、独立密钥生成和重启保留、跨站 / 代理 / 非本机访问拒绝、生产鉴权保留、AI 默认参数和失败提示，以及临时数据目录中的购买保存 / 读取 / 删除流程。测试不会修改现有购买记录，也不会通过 Bark 推送测试记录。
