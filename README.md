...
| `SSQ_ADMIN_TOKEN` | 保护购买记录、核奖及本工具的 AI 管理接口 | 缺失、示例占位值或与 AI 密钥重复时，自动生成独立随机值并保存到权限为 `600` 的 `.env` |

首次接入 DeepSeek 仍需要有效的服务商密钥，程序不能替你生成；缺少它时本地选号和购买记录仍可用，AI 明确显示未配置。

`run_local.sh` 从任意目录启动都会定位到项目目录，启用仅本机的自动连接。该模式同时检查实际监听地址、客户端地址、Host、Origin、浏览器来源及非简单请求头，拒绝代理转发和跨站请求；密钥不会写入 HTML / JavaScript 或本地浏览器存储。公网 / Docker 默认**不启用**这个模式，仍需独立管理密钥鉴权；已验证的远程访问密钥可继续复用。不要为了省输入而把管理接口公开成免鉴权。
