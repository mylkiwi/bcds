...
本地自动连接不保存、不发送管理密钥；页面只显示两项密钥是否已配置。远程服务无法自动授权时才显示访问验证，验证通过后将管理密钥保存在浏览器 `localStorage`，请求时通过 `Authorization: Bearer <token>` 发给服务器。`GET /api/access` 必须经过鉴权，只返回连接模式和配置状态，永不返回密钥值。
